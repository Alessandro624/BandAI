...
class DocumentGeneratorInput(BaseModel):
    ...
    sections: dict[str, str] = Field(..., description="section_title to markdown_content mapping for the document.")
    output_path: str = Field("propolas_output.md", description="File name of the generated document.")


class ComplianceInput(BaseModel):
    tender_raw_text: str = Field(..., description="Raw text of the tender document to analyze.")
    company_profile_json: str = Field(..., description="JSON string containing the company profile information.")
...
class TenderCrawlerTool(BaseTool):
    ...
    description: str = (
        "Crawls a given procuremnent portal and returns all matching tender notices "
        "as a JSON array. Input: portal_name, base_url, keywords, "
        "optional max_results (default 10). Output: JSON array of tender notices with metadata and raw text."
    )
    ...
    def _run(self, portal_name: str, base_url: str, keywords: list[str], max_results: int = 10) -> str:
        # TODO: Implement actual crawling logic here with httpx + BeautifulSoup. For now, we return a mock response.
        mock_response = [
            {
                "portal": portal_name,
...
class ContractDetailTool(BaseTool):
    ...
    description: str = (
        ...
        "Input: contract_id, portal_url. Output: JSON with all available metadata and a completeness score."
    )
    args_schema: Type[BaseModel] = ContractLookupInput

    def _run(self, contract_id: str, portal_url: str) -> str:
        # TODO: Implement actual contract detail lookup logic here (GET https://dati.anticorruzione.it/opendata/dataset/.../contract-id/{contract_id}). For now, we return a mock response.
        mock_response = {
...
class ComplianceCheckerTool(BaseTool):
    """
    Cross-references company certifications and financials against tender
    requirements and returns a structured gap analysis.
    """
    ...
    description: str = (
        ...
        "Input: tender_raw_text, company_profile_json. Output: JSON with lists of met, missing and uncertain requirements."
    )
    args_schema: Type[BaseModel] = ComplianceInput

    def _run(self, tender_raw_text: str, company_profile_json: str) -> str:
        # TODO: Implement actual compliance checking logic here (NLP extraction). For now, we return a mock response.
        analysis = {
            "met": ["Requirement A: Met", "Requirement B: Met", "Experience in past PA: 2 contracts in the last 3 years"],
            "missing": ["Requirement C: Missing", "Certification X: Not held"],
            "uncertain": ["Requirement D: Uncertain", "Financial Stability: Needs Review"],
        }
...
class ProposalWriterTool(BaseTool):
    ...
    description: str = (
        ...
        "section_title to markdown_content. Writes the output to disk at the specified path. "
        "Input: title, sections (dict of section_title to markdown_content), output_path. "
        "Output: Confirmation message with the file path of the generated document."
    )
